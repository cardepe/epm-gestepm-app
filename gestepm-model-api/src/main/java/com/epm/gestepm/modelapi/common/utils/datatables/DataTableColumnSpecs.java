...
public class DataTableColumnSpecs {
	...
	private String sColumns = "columns[";
	...
	private void prepareColumnSpecs(HttpServletRequest request, int i) {
		...
		this.setData(request.getParameter(sColumns + i + "][data]"));
		this.setName(request.getParameter(sColumns + i + "][name]"));
		this.setOrderable(Boolean.valueOf(request.getParameter(sColumns + i + "][orderable]")));
		this.setRegex(Boolean.valueOf(request.getParameter(sColumns + i + "][search][regex]")));
		this.setSearch(request.getParameter(sColumns + i + "][search][value]"));
		this.setSearchable(Boolean.valueOf(request.getParameter(sColumns + i + "][searchable]")));
		...
		String sorttDir = request.getParameter("order[0][dir]");

		if (i == sortableCol) {
			this.setSortDir(sorttDir);
		}
	}
