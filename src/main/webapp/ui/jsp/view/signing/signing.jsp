                            ...
                            <div class="row">
                                <div class="col">
                                    <div class="img-profile-circle float-left">
                                        <img class="user-avatar rounded-circle" src="/ui/static/images/profile.png"
                                             alt="me" height="40"/>
                                    </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form id="userSigningForm" class="needs-validation">
                                ...
                                <input id="usStateInput" type="hidden" name="state"
                                       value="${ userSigningStarted == false ? 0 : 1 }"/>
                                    ...
                                    <div class="col">
                                        <div class="form-group">
                                            <select id="projectDropdown" class="form-control input"
                                                    name="project" ${ currentUser == true && userSigningStarted == false ? '' : 'disabled' }>
                                                <option></option>
                                ...
                                <div class="row">
                                    <div class="col">
                                        <button id="startUserSigningBtn" type="button"
                                                class="btn btn-sm btn-${ userSigningStarted == false ? 'success' : 'danger' } w-100 text-white" ${ currentUser == false ? 'disabled' : '' }>
                                            <i class="fa fa-play" style="font-size: 12px; margin-right: 5px"></i>
                                            <spring:message
                                                    code="signing.page.${ userSigningStarted == false ? 'start' : 'end' }"/>
                                        </button>
                                    </div>
                                </div>
                            </form>
            ...
            <form id="createModalForm" class="needs-validation">
                ...
                <div class="modal-body">
                    ...
                    <div id="materialDiv">
                        <div class="row">
                            <div class="col">
                                <span style="font-size: 14px"><spring:message
                                        code="shares.materials.required.create.info"/></span>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-12 col-md-6 mt-2">
                                ...
                                <div id="mrSignature-pad-op" class="signature-pad">
                                    ...
                                    <div class="signature-pad--footer">
                                        <div class="description"><spring:message
                                                code="shares.intervention.create.signature.two"/></div>
                                        <div class="signature-pad--actions">
                                            <div>
                                                <button id="mrClearSignatureOp" class="clearSignatureButton"
                                                        type="button"><i class="fa fa-redo"></i></button>
                                            </div>
                                            <div></div>
                                        </div>
                                    </div>
                        ...
                        <div id="checkMaterialContentSpanError" class="row mt-2 d-none">
                            <div class="col">
                                <span class="errorSpanText"><spring:message
                                        code="shares.materials.required.create.error"/></span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer clearfix">
                    <div class="w-100">
                        ...
                        <div class="float-right">
                            <button id="createDispShareBtn" type="button" class="btn btn-sm btn-info"><spring:message
                                    code="shares.displacement.create.title"/></button>
                            <button id="createSigningBtn" type="button" class="btn btn-sm btn-success"><spring:message
                                    code="init"/></button>
                        </div>
                    </div>
                </div>
...
<!-- MODAL -->
<div id="createDispModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="createDispForm" class="needs-validation">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                <label for="displacementProjectDropdown" class="col-form-label"><spring:message
                                        code="shares.displacement.table.project.name"/></label>
                        ...
                        <div class="col">
                            <div class="form-group">
                                ...
                                <select id="activityCenter" name="activityCenter" class="form-control"
                                        onchange="loadDisplacement()" required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-6">
                            <label for="displacementType" class="col-form-label"><spring:message
                                    code="displacements.table.displacement.type"/></label>
                            <select id="displacementType" name="displacementType" class="form-control">
                                ...
                                <option value="PUBLIC_TRANSPORT"><spring:message
                                        code="displacements.type.PUBLIC_TRANSPORT"/></option>
                ...
                <div class="modal-footer clearfix">
                    <div class="w-100">
                        ...
                        <div class="float-right">
                            <button id="createDispBtn" type="button" class="btn btn-sm btn-success"><spring:message
                                    code="ccreate"/></button>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#createSigningBtn').click(function () {
            ...
            submitUserSigning(parseInt(state));
        });
    ...
    function submitUserSigning(state) {
        showLoading();
        const createSigningBtn = $('#createSigningBtn');
        createSigningBtn.prop("disabled", true);

        const formData = new URLSearchParams(new FormData(document.getElementById('userSigningForm')));

        axios.post('/signing', formData, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }})
            .then(response => { handleSuccess(response.data, state); })
            .catch(e => showNotify(e.responseText, 'danger'))
            .finally(() => {
    ...
    function handleSuccess(data, state) {
        if (state === 0) {
            if (Boolean(data.isVirtualProject)) {
                changeUserSigningBtn(1);
                reloadTable();
                closeModal();
            } else {
                redirectTo("/shares/intervention");
            }
        } else if (state === 1) {
    ...
    function changeUserSigningBtn(state) {
        ...
        const startUserSigningBtn = $('#startUserSigningBtn');

        projectDropdown.prop('disabled', state === 1);

        startUserSigningBtn
            .toggleClass('btn-success', state === 0)
            .toggleClass('btn-danger', state === 1)
            .text(state === 0
                ? "${jspUtil.parseTagToText('signing.page.start')}"
                : "${jspUtil.parseTagToText('signing.page.end')}"
            );
    }
