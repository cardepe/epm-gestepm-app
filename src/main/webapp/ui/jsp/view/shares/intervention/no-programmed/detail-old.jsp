								...
								<form id="editForm" class="needs-validation">
									<div class="row">
										<div class="col-lg-6">
											<div class="form-group">
												...
												<select id="family" name="familyId" class="form-control" ${ share.family.id == null ? '' : 'disabled' } required>
										...
										<div class="col-lg-6">
											<div class="form-group">
												...
												<select id="subFamily" name="subFamilyId" class="form-control" ${ share.family.id == null ? '' : 'disabled' } required>
													<c:choose>
									...
									<div class="row mt-2">
										<div class="col text-right">
											<a id="postForumUrl" ${ share.family != null ? 'href="${postForumUrl}" target="_blank"' : ''} style="font-size: 12px; font-style: italic"></a>
										</div>
									</div>

									<div class="row mt-2 ${ share.family != null ? 'd-none' : ''}">
										<div class="col text-right">
		...
		if (validateForm()) {
			...
			if (action === 'create')  {
				...
				axios.post(url, formData).then(() => {
					...
					setTimeout(() => { if (goBack) {  window.location.href = lastPageUrl; } else { setStorage(); } }, 100);
...
<script>
	...
	function setReturnButtonUrl() {
		if (document.referrer) {
			...
			} else if (lastPagePath.startsWith('/shares/intervention/no-programmed/detail/')) {
...
<script>
	...
	function init() {
		...
		const subFamilyElement = editForm.querySelector('[name="subFamilyId"]');

		loadPostForumUrl(${share});

		familyElement.onchange = () => {
			updateSubFamilySelect();
			loadPostForumUrl(${share});
		}
		subFamilyElement.onchange = () => { loadPostForumUrl(${share}); }
	}
	...
	function disableEditForm(share) {
	...
	function loadPostForumUrl(share) {
		const form = document.querySelector('#editForm');
		const familyElement = form.querySelector('[name="familyId"]');
		const subFamilyElement = form.querySelector('[name="subFamilyId"]');
		const valid = familyElement.value && subFamilyElement.value;
		const postForumUrl = form.querySelector('#postForumUrl');

		if (valid) {
			const family = familyElement.options[familyElement.selectedIndex].textContent
			const subFamily = subFamilyElement.options[subFamilyElement.selectedIndex].textContent

			postForumUrl.textContent = [share.id, stringToDate(share.noticeDate), family, subFamily].join(' ');
			postForumUrl.href = '${forumUrl}/viewtopic.php?f=' + share.project.forumId + '&t=' + share.topicId;
			postForumUrl.target = '_blank';
		} else {
			postForumUrl.textcontent = '-';
			postForumUrl.href = undefined;
			postForumUrl.target = undefined;
		}
	}
